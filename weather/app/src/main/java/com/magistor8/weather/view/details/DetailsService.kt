...
class DetailsService(name: String = "DetailService") : IntentService(name) {
    ...
    private fun loadWeather(lat: String, lon: String) {
        try {
            ...
            try {
                ...
                val weatherDTO = Gson().fromJson(BufferedReader(InputStreamReader(urlConnection.inputStream)), WeatherDTO::class.java)
                onResponse(weatherDTO)
            } catch (e: Exception) {
                onErrorRequest(e.message ?: getString(R.string.ErrorConnection))
            } finally {
    ...
    private fun onSuccessResponse(weatherDTO: WeatherDTO) {
        broadcastIntent.putExtra(DETAILS_LOAD_RESULT_EXTRA,weatherDTO)
